...
class ListaPersonagem {
    ...
    public Personagem get(int index) {
        ...
        CelulaLista current = primeiro.prox;
    ...
    public void set(int index, Personagem personagem) {
        if (index < 0 || index >= this.tam) {
            throw new IndexOutOfBoundsException("Index fora dos limites: " + index);
        }
        CelulaLista current = primeiro.prox;
    ...
    public Personagem RemoverFim() throws Exception {
        ...
        Personagem personagem = ultimo.personagem;
        ultimo = ultimo.ant;
...
public class Personagem {
    ...
    public static void QuickSort(ListaPersonagem lista, int esq, int dir) {
        if (esq < dir) {
            int indicePivot = partition(lista, esq, dir);
            QuickSort(lista, esq, indicePivot - 1);
            QuickSort(lista, indicePivot + 1, dir);
        }
    }

    private static int partition(ListaPersonagem lista, int esq, int dir) {
        Personagem pivot = lista.get(dir);
        int i = esq - 1;

        for (int j = esq; j < dir; j++) {
            if (CompareHouseOrName(lista.get(j), pivot) <= 0) {
                i++;
                swap(lista, i, j);
            }
        }
    ...
    public static void swap(ListaPersonagem lista, int i, int j) {
