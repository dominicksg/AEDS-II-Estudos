...
Personagem *desenfileirarFim(ListaPersonagem *fila)
{
    if (fila->tam == 0)
    {
    ...
    Personagem *tmp = fila->array[fila->tail];
    fila->tail = (fila->tail - 1 + fila->maxTam) % fila->maxTam;
    fila->tam--;
    return tmp;
}
...
int main()
{
    ...
    inicializarListaPersonagem(&lista, 5);
    ...
    {
        printf("[%d ## ", i);
        imprimir(lista.array[(i + lista.head) % lista.maxTam]);
        free(lista.array[(i + lista.head) % lista.maxTam]);
    }
    printf("[ Tail ]\n");
