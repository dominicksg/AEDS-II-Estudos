...
class Personagem {
    ...
    public static int PesquisaSequencial(Personagem obj[], String nome) {
        Boolean check = false;
        int comparacoes = 0;

        for (int i = 0; i < obj.length; i++) {
            if (obj[i] != null && obj[i].getName().equals(nome)) {
                check = true;
                i = obj.length;
            }
            comparacoes++;
    ...
    public static void log(long tempoExecucao, int comparacoes) {
        File log = new File("831566_sequencial.txt");
        double segundos = tempoExecucao / 1_000_000_000.0;

        try {
            PrintWriter writer = new PrintWriter(new FileWriter(log, true));
            writer.println("831566\t" + segundos + "\t" + comparacoes);
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) {
        ...
        while (ID.equals(nome) != true) {
            comparacoes += PesquisaSequencial(personagem, nome);
            nome = sc.nextLine();
        }
