...
int *bubbleSort(Personagem **vetor, int n, int info[])
{
    int i, j;
    for (i = 0; i < n - 1; i++)
    {
        ...
        {
            info[0]++;
            // Comparação de hairColour
            if (strcmp(vetor[j]->hairColour, vetor[j + 1]->hairColour) > 0)
            {
...
void registraLog(int tempoExecucao, int info[])
{
    ...
    logFile = fopen("831566_bubblesort.txt", "a");
...
int main()
{
    ...
    bubbleSort(personagens, count, info);
