...
void selectionSortRecursivo(Personagem **vetor, int n, int i, int info[])
{
    if (i >= n - 1)
        return;
    ...
    selectionSortRecursivo(vetor, n, i + 1, info);
}

void selectionSort(Personagem **vetor, int n)
{
    int info[2] = {0};
    selectionSortRecursivo(vetor, n, 0, info);
}
...
void registraLog(int tempoExecucao, int info[])
{
    ...
    logFile = fopen("831566_selecaoRecursiva.txt", "a");
...
int main()
{
    ...
    selectionSortRecursivo(personagens, count, 0, info);
