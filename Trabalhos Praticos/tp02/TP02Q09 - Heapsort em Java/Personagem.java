...
class Personagem {
        ...
        for (int i = 0; i < vetor.length - 1 && vetor[i] != null; i++) {
            ...
            for (int j = i + 1; j < vetor.length; j++) {
                if (vetor[j] != null && vetor[j].getName().compareTo(maior) < 0
                        && vetor[j].getDateOfBirth().equals(vetor[i].getDateOfBirth())) {
    ...
    public static void InsertionSort(Personagem[] vetor) {
        for (int i = 10; i < vetor.length && vetor[i] != null; i++) {
            Personagem tmp = vetor[i];
            Date keyDateOfBirth = vetor[i].getDateOfBirth();
            String keyName = vetor[i].getName();
            int j = i - 1;

            while (j >= 0 && vetor[j].getDateOfBirth().compareTo(keyDateOfBirth) >= 0) {
                vetor[j + 1] = vetor[j];
                j--;
            }
            vetor[j + 1] = tmp;
    ...
    public static int CompareHouseOrName(Personagem a, Personagem b) {
        int houseComparison = a.getHairColour().compareTo(b.getHairColour());

        // A resposta da pergunta (condicao?) é verdadeira ? primeiro valor : senao
        // segundo valor
        return (houseComparison != 0) ? houseComparison : a.getName().compareTo(b.getName());
    }

    public static void ConstroiHeap(Personagem heap[], int pos, int[] info) { // Adiciona no heap
        while (pos > 1 && CompareHouseOrName(heap[pos], heap[pos / 2]) > 0) {
            swap(heap, pos, pos / 2);
            pos = pos / 2;
            info[0]++;
            info[1]++;
    ...
    public static int numObjetos(Personagem vetor[]) {
        int count = 0;
    ...
    public static int getMaiorFilho(Personagem heap[], int i, int tam) {
        ...
        if (2 * i + 1 > tam) {
            ...
            if (CompareHouseOrName(heap[2 * i], heap[2 * i + 1]) < 0) {
                maior = 2 * i + 1;
            } else {
                maior = 2 * i;
            }
    ...
    public static void ReconstroiHeap(Personagem heap[], int tam, int[] info) {
        int i = 1;
        while (hasFilho(heap, i, tam) == true) {
            int filho = getMaiorFilho(heap, i, tam);

            if (CompareHouseOrName(heap[i], heap[filho]) < 0) {
                swap(heap, i, filho);
                i = filho;
                info[0]++;
                info[1]++;
    ...
    public static int[] Heapsort(Personagem p[]) {
        int[] info = new int[2]; // info[0] = comparacoes, info[1] = movimentacoes

        int tam = 0;
        Personagem heap[] = new Personagem[numObjetos(p) + 1];

        // Contrução do heap
        for (int i = 0; p[i] != null && i < numObjetos(p); i++) {
            heap[i + 1] = p[i].clone(p[i]);
            ConstroiHeap(heap, i + 1, info);
            tam++;
        }

        while (tam > 1) {
            swap(heap, 1, tam--);
            ReconstroiHeap(heap, tam, info);
        }
    ...
    public static void log(long tempoExecucao, int info[]) {
        File log = new File("831566_heapsort.txt");
    ...
    public static void main(String[] args) {
        ...
        info = Heapsort(personagem);

        // SelectionSort(personagem);
