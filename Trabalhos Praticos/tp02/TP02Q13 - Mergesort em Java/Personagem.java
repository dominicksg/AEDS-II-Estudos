...
class Personagem {
    ...
    // String[] L = new String[n1 + 1]; // Subarray esquerdo +1 para a sentinela
    // String[] R = new String[n2 + 1]; // Subarray direito +1 para a sentinela

    // // Copia os dados para os subarrays temporários
    // for (int i = 0; i < n1; i++)
    // L[i] = vetor[esq + i].getActorName();
    // for (int j = 0; j < n2; j++)
    // R[j] = vetor[meio + 1 + j].getActorName();
    ...
    // vetor[k].setActorName(L[i]);
    // i++;
    // } else {
    // vetor[k].setActorName(R[j]);
    // j++;
    // }
    // k++;
    // }
    ...
    public static int[] Mergesort(int esq, int dir, Personagem vetor[], int info[]) {
        if (esq < dir) {
            int meio = (esq + dir) / 2;
            Mergesort(esq, meio, vetor, info);
            Mergesort(meio + 1, dir, vetor, info);
            intercalar(vetor, esq, meio, dir, info);
        }
        return info;
    }

    public static int[] intercalar(Personagem vetor[], int esq, int meio, int dir, int info[]) {
        // Definir tamanho dos dois subarrays
        int n1 = meio - esq + 1;
        int n2 = dir - meio;

        // Criação dos subarrays temporários
        ...
        // Inicializa as sentinelas
        // L[n1] = new Personagem(); // Sentinela
        // R[n2] = new Personagem(); // Sentinela
        ...
        while (i < n1 && j < n2) {
            info[0]++;
            if (L[i].getActorName().compareTo(R[j].getActorName()) <= 0) {
                vetor[k] = L[i];
                i++;
            } else {
                vetor[k] = R[j];
    ...
    public static void log(long tempoExecucao, int info[]) {
        File log = new File("831566_mergesort.txt");
    ...
    public static void main(String[] args) {
        ...
        info = Mergesort(0, i - 1, personagem, info);
