...
class Personagem {
    ...
    public static int[] SelectionSort(Personagem vetor[], int info[]) {
    ...
    public static int getMaxYear(Personagem[] personagens) {
        int maxYear = personagens[0].yearOfBirth;

        for (int i = 1; i < personagens.length; i++) {
            if (personagens[i] != null && personagens[i].yearOfBirth > maxYear) {
                maxYear = personagens[i].yearOfBirth;
            }
        }
    ...
    public static int[] CountingSort(Personagem[] personagens, int info[]) {
        // Determina o maior ano de nascimento
        int maxYear = getMaxYear(personagens);
        ...
        // Conta as ocorrências de cada ano de nascimento
        ...
        // Transforma count para armazenar as posições
        for (int i = 1; i < count.length; i++) {
            count[i] += count[i - 1];
        }

        // Cria o array para os personagens ordenados
        Personagem[] sorted = new Personagem[personagens.length];
        for (int i = personagens.length - 1; i >= 0; i--) {
            if (personagens[i] != null) {
                sorted[count[personagens[i].yearOfBirth] - 1] = personagens[i];
                count[personagens[i].yearOfBirth]--;
                info[1]++;
            }
        }
        ...
        info[0] += count.length - 1; // Loop de inicialização
        info[0] += count.length - 1; // Loop de transformação

        return info;
    }

    public static void log(long tempoExecucao, int info[]) {
        File log = new File("831566_countingsort.txt");
    ...
    public static void main(String[] args) {
        ...
        info = SelectionSort(personagem, info); // eu sei que isso fica mais custoso,
        // pretendo melhorar futuramente

        info = CountingSort(personagem, info);

        for (i = 0; personagem[i] != null && i < 30; i++) {
            personagem[i].imprimir();
        }
