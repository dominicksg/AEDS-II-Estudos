...
int comparaEyeNome(Personagem *vetor, Personagem *tmp)
{
    int comparacao = strcmp(vetor->eyeColour, tmp->eyeColour);
    return comparacao != 0 ? comparacao : strcmp(vetor->name, tmp->name);
}

void insercaoPorCor(Personagem **vetor, int n, int cor, int h, int info[])
{
    for (int i = (h + cor); i < n; i += h)
    {
        Personagem *tmp = vetor[i];
        int j = i - h;
        while ((j >= 0) && (comparaEyeNome(vetor[j], tmp)) > 0)
        {
            vetor[j + h] = vetor[j];
            j -= h;
            info[0]++;
...
int *shellSort(Personagem **vetor, int n, int info[])
{
    int h = 1;
    do
    {
    ...
    do
    {
        ...
        {
            insercaoPorCor(vetor, n, cor, h, info);
        }
    } while (h != 1);
    return info;
}
...
void registraLog(int tempoExecucao, int info[])
{
    ...
    // Abre o arquivo em modo de escrita, acrescentando ao final se existir ou criando um novo se não existir
    logFile = fopen("831566_shellsort.txt", "a");
...
int main()
{
    ...
    shellSort(personagens, count, info);
